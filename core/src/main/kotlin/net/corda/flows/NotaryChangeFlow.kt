...
object NotaryChangeFlow : AbstractStateReplacementFlow() {
    ...
    class Acceptor(otherSide: Party,
        ...
        override fun verifyProposal(proposal: AbstractStateReplacementFlow.Proposal<Party>): Unit {
            ...
            checkProposal(proposedTx.type is TransactionType.NotaryChange) {
            ...
            checkProposal(isNotary) { "The proposed node $newNotary does not run a Notary service " }
            checkProposal(state in proposedTx.inputs) {
                "The proposed state $state is not in the proposed transaction inputs"
            }
